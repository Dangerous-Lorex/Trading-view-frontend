...
export class DashboardChartsData {
  ...
  initMainChart(period: string = 'Month') {
    ...
    const maxData1 = Math.max(...data1) + 20;
    const maxData2 = Math.max(...data2) + 20;
    for (let i = 0; i <= this.mainChart['elements']; i++) {
    ...
    const options = {
      ...
      scales: {
        ...
        y: {
          ...
          min: -100,
          ticks: {
            stepSize: 10, // Set step size to 10 for each 10%
            callback: (value: any) => `${value}%` // Format labels as percentages
          },
          grid: {
            drawOnChartArea: true // Added to show grid
          }
        }
      },
      elements: {
